...
Setup:
    ...
    4. Run: python discord_keyword_bot.py
"""
...
# ── AI detection helper ───────────────────────
async def check_ai_text(text: str) -> dict:
    """
    Sends the application text to Claude via the Anthropic API and asks it
    to determine whether the text was likely AI-generated.

    Returns a dict with:
        verdict  : "AI-Generated" | "Human-Written" | "Uncertain"
        confidence: "High" | "Medium" | "Low"
        reasoning: str  — a short explanation of the verdict
    """
    prompt = f"""You are an expert at detecting AI-generated text. Analyse the following application text and determine whether it was written by a human or generated by an AI such as ChatGPT, Claude, or similar tools.

Look for these AI indicators:
- Overly formal or polished language with no personal voice
- Generic, vague statements that could apply to anyone
- Perfectly structured paragraphs with no natural flow breaks
- Buzzwords and filler phrases (e.g. "I am passionate about", "I would be a great asset")
- Lack of specific personal anecdotes or typos
...
@tree.command(name="ai_check", description="Check whether an application was written by AI or a human.")
@app_commands.describe(application="Paste the full application text here.")
@app_commands.checks.has_permissions(manage_messages=True)
async def ai_check(interaction: discord.Interaction, application: str):
    """
    Analyses the pasted application text using Claude and returns a verdict:
      - AI-Generated  → text is likely written by an AI tool
      - Human-Written → text appears to be written by a real person
      - Uncertain     → not enough signal to make a confident call

    Restricted to members with the Manage Messages permission (i.e. mods).
    Result is visible to everyone in the channel.
    """
    # Defer publicly so everyone can see the bot is working
    await interaction.response.defer()

    if not ANTHROPIC_API_KEY:
        await interaction.followup.send(
            "⚠️ `ANTHROPIC_API_KEY` environment variable is not set. Add it in Railway → Variables."
        )
        return

    try:
        result = await check_ai_text(application)
    except Exception as e:
        await interaction.followup.send(f"⚠️ AI check failed: `{e}`")
        return
    ...
    # Pick colour and emoji based on verdict
    ...
    embed = discord.Embed(
        title=f"{emoji} AI Detection Result: {verdict}",
        color=colour,
    )
    ...
    embed.add_field(
        ...
        # Show first 300 chars so the embed doesn't get too long
        value=f"```{application[:300]}{'...' if len(application) > 300 else ''}```",
        inline=False,
    )
    embed.set_footer(text="Powered by Claude • Results are indicative, not definitive.")

    await interaction.followup.send(embed=embed)


@ai_check.error
async def ai_check_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
